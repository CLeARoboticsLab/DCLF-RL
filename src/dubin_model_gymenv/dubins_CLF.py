...
parser.add_argument('--num_steps', type=int, default=35000, metavar='N',
...
class DubinGym(gym.Env):
	...
	def __init__(self, start_point):
		...
		self.action_space = spaces.Box(np.array([0., -1.5]), np.array([1., 1.5]), dtype = np.float32) # Action space for [throttle, steer]
	...
	def reset(self): 
		x = random.uniform(-1.5, 1.5) # Reset to random point on unit circle
		y = random.choice([-1.5, 1.5])*math.sqrt(2.25 - x**2) # Reset to random point on unit circle
		#x=-3+random.random()
		#y=-3+random.random()
	...
	def step(self,action):
		...
		CLVF_0=get_value_lyapunov(pos_x,pos_y,pos_theta)
		self.pose = self.update_state(self.pose, action, 0.005) # 0.005 Modify time discretization
		CLVF_1=get_value_lyapunov(pos_x,pos_y,pos_theta)
		...
		if ((abs(self.pose[0]) < 1.) and (abs(self.pose[1]) < 1.)):
			...
			if(abs(self.pose[0]-self.target[0])<THRESHOLD_DISTANCE_2_GOAL and  abs(self.pose[1]-self.target[1])<THRESHOLD_DISTANCE_2_GOAL):
				...
				reward = self.get_reward()+(CLVF_0-CLVF_1)
	...
	def render(self,mode):
		...
		if mode==1:
			arrowsx.append(self.pose[0]*MAX_X)
			arrowsy.append(self.pose[1]*MAX_Y)
			dirx.append(math.cos(self.pose[2]))
			diry.append(math.sin(self.pose[2]))
		...
		plt.quiver(arrowsx, arrowsy, dirx, diry,scale=50, color= 'blue', zorder=10)
		plt.plot(self.traj_x, self.traj_y,"ob", color='lightblue', markersize = 2, label="trajectory",zorder=1)
	...
	def plot_car(self, cabcolor="-r", truckcolor="-k"):  # pragma: no cover
		...
		#fr_wheel=np.copy(rr_wheel)
		#fl_wheel=np.copy(rl_wheel)
		#fr_wheel[1, :] += WB
		#fl_wheel[1, :] += WB
...
def main():
	...
	for i_episode in itertools.count(1):
		...
		while not done:
			...
			if num_goal_reached>2:
				
				if total_numsteps%80==0:
					mode=1
				else:
					mode=0

				env.render(mode) # Rendering toggle
...
if __name__ == '__main__':
	for i in range (1):
		...
		filename='reward_CLVF_full_SAC'+str(i)+'.txt'
		#np.savetxt("/home/al55293/RL-with-Lyapunov-functions/dubin_model_gymenv/rewards_data/"+filename,mu1)
