...
%     .stopDiverge:         (bool) set to treu to stop the computation when
%                           it diverge
%     .divergeThreshold:    Max value of the value function. When larger
%                           than this value, view that point as diverged 
...
if (isfield(extraArgs, 'visualize') && isstruct(extraArgs.visualize))...
    ...
    if isfield(extraArgs.visualize, 'targetSet') ...
        ...
        if ~isfield(extraArgs.visualize,'plotColorTS')
            extraArgs.visualize.plotColorTS = 'r';
        end
    ...
    if isfield(extraArgs.visualize, 'targetFunction') &&...
        ...
        if ~isfield(extraArgs.visualize,'plotColorTF')
            extraArgs.visualize.plotColorTF = 'r';
        end
...
stopDiverge = false;
if isfield(extraArgs, 'stopDiverge')
    stopDiverge = extraArgs.stopDiverge;
    if isfield(extraArgs, 'divergeThreshold')
        divergeThreshold = extraArgs.divergeThreshold;
    else
        divergeThreshold = 1e-5;
    end
end
...
if isfield(extraArgs, 'stopConvergeTTR')
    ...
    if isfield(extraArgs, 'convergeThresholdTTR')
...
if isfield(extraArgs, 'computeTTR') && extraArgs.computeTTR
    if i > istart
        schemeDataTTR.ttrLast = schemeDataTTR.ttr;
    else
        schemeDataTTR = schemeData;
        schemeDataTTR.ttrLast = ones(size(data0(:))).*inf;
    end
    [ ~, schemeDataTTR ] = feval(@postTimestepTTR, tNow, y, schemeDataTTR);
    changeTTR = max(abs(schemeDataTTR.ttr(:)-schemeDataTTR.ttrLast(:)));
end
...
%         if isfield(extraArgs, 'stopDiverge') && extraArgs.stopDiverge == 1 
% 
%             divergeThreshold = extraArgs.divergeThreshold;
% 
%             Large = find(y >= divergeThreshold);
%             y(Large) = divergeThreshold;
%             [tNow, y, schemeData] = feval(integratorFunc, schemeFunc, [tNow tau(i)],...
%                 y, integratorOptions, schemeData);
%         else  
            [tNow, y, schemeData] = feval(integratorFunc, schemeFunc, [tNow tau(i)],...
                y, integratorOptions, schemeData);
            disp(tPrev);
...
%         end
        ...
        if ~isfield(extraArgs, 'discountMode') || ...
            ...
            elseif strcmp(compMethod, 'minCLF')
                dt = tNow - tPrev;

                y = max(y ./(1-schemeData.clf.gamma*dt), targets(:));
                if stopDiverge
                    Div = find(y>divergeThreshold);
                    y(Div) = divergeThreshold;
                end
...
%     if stopDiverge
%         Deriv = computeGradients(g, data_i);
%         Grad = eval_u(g,Deriv,g.xs);
%         Max_D = max(Grad,[],'all');
%     end
...
%             changeD = Vc(i);
%         else
%             changeD = abs(Vc(i)-Vc(i-1));
%         end
%         if ~quiet
%             fprintf('Largest Value that Converged: %f\n', Vc(i))
%             fprintf('the change is: %f\n', changeD)
%         end
%     end
    ...
    if stopConverge && change < convergeThreshold
     ...
     %% Stop computation if the ROA converged
%      if stopDiverge && Max_D > divergeThreshold
% 
%          extraOuts.stoptau = tau(i);
%          tau(i+1:end) = [];
% 
...
if isfield(extraArgs, 'computeTTR') && extraArgs.computeTTR
    extraOuts.ttr = reshape(schemeDataTTR.ttr, g.shape);
